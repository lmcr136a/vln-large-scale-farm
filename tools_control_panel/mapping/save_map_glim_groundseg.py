...
def render_and_save(grid, z_rel, meta, path_xyz, robot_yaw, output_path, world_rot_angle=0.0, robot_pos=None,
                    ground_xyz=None, traversable_cells=None, raw_gps_xyz=None):
    try:
        u_min = meta['u_min']
        v_min = meta['v_min']
        gs    = meta['grid_size']
        ...
        if raw_gps_xyz is not None and len(raw_gps_xyz) > 1:
            gps_px = [to_px(p[0], p[1]) for p in raw_gps_xyz]
            draw.line(gps_px, fill=COLOR_RAW_GPS, width=lw)
...
class ContinuousPointCloudMapper(Node):
    ...
    def __init__(self, cfg: dict):
        ...
        self.raw_gps_path      = np.empty((0, 3))
        ...
        self.raw_gps_sub = self.create_subscription(
            Path, '/glim_ros/raw_gps_world', self.raw_gps_callback, odom_qos)
    ...
    def raw_gps_callback(self, msg: Path):
        """Receive raw GPS path from /glim_ros/raw_gps_world (already in map frame)."""
        if not msg.poses:
            return
        self.raw_gps_path = np.array([[p.pose.position.x,
                                       p.pose.position.y,
                                       p.pose.position.z] for p in msg.poses], dtype=np.float64)
