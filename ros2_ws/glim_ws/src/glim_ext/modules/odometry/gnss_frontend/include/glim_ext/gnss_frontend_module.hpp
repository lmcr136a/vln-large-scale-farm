...
public:
      ...
      [this](const geometry_msgs::msg::PoseStamped::ConstSharedPtr msg) {
        ...
        if (!T_sg_set) {
          ...
          if (aligned) {
            ...
            logger->info("T_world_enu remapped to saved map frame (post-alignment)");
          } else {
  ...
  virtual std::vector<GenericTopicSubscription::Ptr> create_subscriptions() override {
    const auto gps_sub = std::make_shared<TopicSubscription<NavSatFix>>(
      gnss_topic, [this](const NavSatFixConstPtr msg) { gnss_callback(msg); });
    return {gps_sub};
  }
...
private:
  ...
  void task() {
    ...
    while (!kill_switch) {
      ...
      while (!frame_queue.empty() && frame_queue.front()->stamp <= enu_queue.back()[0]) {
        ...
        const auto right = std::lower_bound(enu_queue.begin(), enu_queue.end(), stamp,
          [](const Eigen::Vector4d& v, double t) { return v[0] < t; });
        if (right == enu_queue.begin() || right == enu_queue.end()) break;

        const auto left = right - 1;
        const double tl = (*left)[0], tr = (*right)[0];
      ...
      if (!aligned && (int)match_est.size() >= min_matches &&
          (match_est.front() - match_est.back()).norm() > min_baseline) {
        estimate_alignment();
      ...
      if (aligned) {
        ...
        for (size_t i = num_factored; i < match_enu.size(); i++) {
          if (latest_frame_id - match_ids[i] > max_factor_lag_frames) continue;
          ...
          output_factors.push_back(gtsam::NonlinearFactor::shared_ptr(
            new gtsam::PoseTranslationPrior<gtsam::Pose3>(X(match_ids[i]), xyz, model)));
  ...
  void estimate_alignment() {
    ...
    if (svd.matrixU().determinant() * svd.matrixV().determinant() < 0.0) S(1, 1) = -1;
